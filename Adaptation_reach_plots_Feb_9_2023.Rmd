...
demo.vars <- ft.data %>% filter(TN <= 30) %>%
  group_by(Subject.ID) %>%
...
```
#Adaptation averages across sessions and phase

```{r}

repeat_sessions <- ft.data.wrepeat %>% 
  mutate(timesplayed = case_when(repeat. == "First Time" ~ 1, 
                                 repeat. == "Second Time" ~ 2, 
                                 repeat. == "Third Time" ~ 3, 
                                 repeat. == "More than three" ~ 4),
        phase = case_when(TN >= 31 & TN <= 40 ~ "Early", 
                           TN >= 74 & TN <= 84 ~ "Late", 
                           TN >= 85 & TN <= 90 ~ "Aftereffect")) 


repeat_sessions$phase <- factor(repeat_sessions$phase, levels = c("Early", "Late", "Aftereffect"))

across_sessions <- repeat_sessions %>% 
  group_by(Subject.ID, timesplayed, phase) %>%
  dplyr::summarise(Hand_mean = mean(HandFlip, na.rm = TRUE)) %>% 
  drop_na(phase) %>% 
  ggplot(aes(x = timesplayed, y = Hand_mean)) + 
  stat_summary(fun.data = "mean_se", geom = "pointrange", size = 0.2) + 
  stat_summary(fun = "mean", geom = "line") + 
  coord_capped_cart(ylim = c(0, 45)) + 
  facet_rep_wrap(.~phase, repeat.tick.labels = TRUE) + 
  scale_y_continuous(breaks = c(0, 15, 30, 45)) + 
  theme_minimal() +  
  theme(strip.background = element_blank(),
  strip.text.x = element_blank()) + 
  labs(y  = "Hand Angle (°)", x = "Session") 
...
#Distribution of Reaches per phase
```{r}
...
my.pro.data.phase <- ft.data %>% 
  ...
  dplyr::summarise(Hand_mean = mean(HandFlip, na.rm = TRUE)) %>% 
  drop_na() %>% 
  mutate(mean_line = case_when(phase == "Early" ~ 22.27, 
                               phase == "Late" ~ 34.5, 
                               phase == "Aftereffect" ~ 12.6))

my.pro.data.phase$phase <- factor(my.pro.data.phase$phase, levels = c("Early", "Late", "Aftereffect"))

mygrouphist <- my.pro.data.phase %>% 
  ggplot(aes(x = Hand_mean)) +
  geom_histogram(fill = "grey60", binwidth = 2.5, alpha = 0.75) + 
  #geom_density(fill = "grey60") +
  facet_rep_wrap(phase~., ncol = 3, repeat.tick.labels = TRUE) +  labs(x = "Hand Angle (°)", y = "# of People") +
  geom_vline(aes(xintercept = mean_line), color = "black", linetype = "dashed", alpha = 0.5, size = 0.2) + 
  theme(strip.background = element_blank(),
  strip.text.x = element_blank()) +
  scale_x_continuous(breaks = c(0, 45)) + 
  coord_capped_cart(xlim = c(-20, 60)) +
  theme_minimal()
print(mygrouphist)

std.error(my.pro.data.phase$Hand_mean[my.pro.data.phase$phase == "Early"])
std.error(my.pro.data.phase$Hand_mean[my.pro.data.phase$phase == "Late"])
std.error(my.pro.data.phase$Hand_mean[my.pro.data.phase$phase == "Aftereffect"])
...
fast_mt <- demo.vars$Subject.ID[demo.vars$MT_mean_baseline < median(demo.vars$MT_mean_baseline) ]

MTdata <- ft.data %>%
 mutate(Above = ifelse(Subject.ID %in% fast_mt, 'Fast', 'Slow')) %>% 
...
low_var <- demo.vars$Subject.ID[demo.vars$Var_sum_baseline < median(demo.vars$Var_sum_baseline) ]

vardata <- ft.data %>%
 mutate(Above = ifelse(Subject.ID %in% low_var, 'Low Variability', 'High Variability')) %>% 
  group_by(TN, Block, FB, Above) %>%
  dplyr::summarise(Hand_mean = mean(HandFlip, na.rm = TRUE), Hand_sem = std.error(HandFlip, na.rm = TRUE) )
...
fast_st <- demo.vars$Subject.ID[demo.vars$ST_mean_baseline < median(demo.vars$ST_mean_baseline) ]

STdata <- ft.data %>%
 mutate(Above = ifelse(Subject.ID %in% fast_st, 'Fast', 'Slow')) %>% 
  group_by(TN, Block, FB, Above) %>%
  dplyr::summarise(Hand_mean = mean(HandFlip, na.rm = TRUE), Hand_sem = std.error(HandFlip, na.rm = TRUE) )
...
fast_rt <- demo.vars$Subject.ID[demo.vars$RT_mean_baseline < median(demo.vars$RT_mean_baseline) ]

RTdata <- ft.data %>%
  mutate(Above = ifelse(Subject.ID %in% fast_rt, 'Fast', 'Slow')) %>% 
  group_by(TN, Block, FB, Above) %>%
  dplyr::summarise(Hand_mean = mean(HandFlip, na.rm = TRUE), Hand_sem = std.error(HandFlip, na.rm = TRUE) )
...
small_screen_subs <- ft.data$Subject.ID[ft.data$screensize < median(ft.data$screensize) ]

screendata <- ft.data %>%
 mutate(Above = ifelse(Subject.ID %in% small_screen_subs, 'Small', 'Large')) %>% 
  group_by(TN, Block, FB, Above) %>%
  dplyr::summarise(Hand_mean = mean(HandFlip, na.rm = TRUE), Hand_sem = std.error(HandFlip, na.rm = TRUE) )
